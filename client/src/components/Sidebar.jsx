...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
    const { navigate, user, chats, setSelectedChat, theme, setTheme, createNewChat, fetchUsersChat, setToken, axios, setChats, token } = useAppContext()
  ...
  <div className={`flex flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'} `}>
            ...
            <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-2 pr-3 sidebar-scroll  '>
                {chats && chats.filter((chat) => chat.messages?.[0] ? chat.messages[0]?.content.toLowerCase().includes(search.toLowerCase()) : chat.name?.toLowerCase().includes(search.toLowerCase())).map((chat) => (
                    <div onClick={() => { navigate('/'); setSelectedChat(chat); setIsMenuOpen(false) }} key={chat._id} className='p-2 px-4 dark:bg-[#57317C]/10 border border-gray-300 dark:border-[#80609F]/15 rounded-md cursor-pointer flex justify-between group   transition-all ease-in-out duration-300 dark:hover:bg-primary/20 hover:bg-blue-100   '>
                        <div>
                            <p className='truncate w-full'>
                                {chat.messages && chat.messages.length > 0 ? chat.messages[0]?.content?.slice(0, 32) : chat.name}
                            </p>
                            <p className='text-xs text-gray-500 dark:text-[#B1A6C]'>
            ...
            <div onClick={() => { navigate('/credit'); setIsMenuOpen(false) }} className='flex items-center gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
